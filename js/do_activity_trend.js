...
class DOActivityTrend {
    ...
    async processSourceFile(file) {
        try {
            ...
            if (!dateStr) {
                const manual = await window.showPromptDialog({
                    title: "Missing Date",
            ...
            for (let i = headerIdx + 1; i < dataEndIdx; i++) {
                ...
                if (rem.includes("LCL")) {
                    ...
                    lclQty += v9;
                    lclSku += v10;
                    lclDetails.push({ do: inv, sku: v10, qty: v9 });
                }
            }
    ...
    async exportToExcel() {
        if (this.history.length === 0) {
            showToast("No activity history to export! Please import daily reports first.", "warning");
            return;
        }
        ...
        // Sheet 3: LCL_DO_Details (Individual DO rows for LCL)
        const lclDetailsData = [["Date", "LCL DO Number", "SKU", "QTY"]];
        this.history.forEach(h => {
            if (h.lclDetails && h.lclDetails.length > 0) {
                h.lclDetails.forEach(detail => {
                    lclDetailsData.push([h.date, detail.do, detail.sku, detail.qty]);
                });
            }
        });
        const wsLclDetails = XLSX.utils.aoa_to_sheet(lclDetailsData);
        wsLclDetails['!cols'] = [{ wch: 12 }, { wch: 18 }, { wch: 8 }, { wch: 8 }];
        XLSX.utils.book_append_sheet(workbook, wsLclDetails, "LCL_DO_Details");

        // --- Apply Professional Styling to All Sheets ---
        const applyProfessionalStyles = (ws) => {
            if (!ws['!ref']) return;
            ...
            for (let R = range.s.r; R <= range.e.r; ++R) {
                for (let C = range.s.c; C <= range.e.c; ++C) {
                    ...
                    ws[cellRef].s = {
                        ...
                        fill: {
                            fgColor: { rgb: isHeader ? "0F172A" : (R % 2 === 0 ? "F8FAFC" : "FFFFFF") }
                        },
                        alignment: {
                            vertical: "center",
                            horizontal: C === 0 ? "left" : "center" // Header and data nicely centered/left
                        },
                        border: {
        ...
        applyProfessionalStyles(wsSummary);
        applyProfessionalStyles(wsLcl);
        applyProfessionalStyles(wsLclDetails);
        ...
        try {
            ...
            showToast("Could not save the file. Check browser permissions.", "error");
        }
    }

    // Export historical trend data to JSON format for sharing between users and backup
    exportToJSON() {
        ...
        // Guarantee chronological date sorting before exporting
        const sortedHistory = [...this.history].sort((a, b) => a.date.localeCompare(b.date));

        const payload = {
            app: "DO_Status_Hub",
            version: "1.0",
            type: "activity_trend_data",
            exportedAt: new Date().toISOString(),
            timeframe: this.timeframe || "30",
            chartType: this.chartType || "main",
            recordsCount: sortedHistory.length,
        ...
        a.href = url;
        a.download = `DO_Activity_Trend_Data_${dateStr}.json`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
    ...
    // Import shared historical trend data from JSON
    importFromJSON(event) {
        ...
        reader.onload = async (e) => {
            try {
                ...
                } else if (content && Array.isArray(content.history)) {
                    importedList = content.history;
                } else {
                    showToast("Invalid JSON structure. The file must contain an activity history array or a 'history' property.", "error");
                    return;
                }
                ...
                importedList.forEach(item => {
                    if (item && item.date && dateRegex.test(String(item.date).trim())) {
                        cleanRecords.push({
                            ...
                            lclDo: Number(item.lclDo) || 0,
                            lclSku: Number(item.lclSku) || 0,
                            lclVol: Number(item.lclVol) || 0,
                            lclQty: Number(item.lclQty) || 0,
                            lclDetails: Array.isArray(item.lclDetails) ? item.lclDetails : []
                        });
                    }
                });

                if (cleanRecords.length === 0) {
                    showToast("No valid daily activity records (with YYYY-MM-DD date format) found in the JSON file.", "warning");
                    return;
                }

                // Prompt user to choose between Merge and Replace
                const merge = await window.showConfirmDialog({
                    title: "Import Shared History",
                    message: `Found ${cleanRecords.length} daily activity record(s) in JSON file.\n\n` +
                             `Click [MERGE] to UPDATE with your current history (updates matching dates, appends new dates).\n` +
                ...
                if (merge) {
                    ...
                    cleanRecords.forEach(rec => {
                        const existingIdx = this.history.findIndex(h => h.date === rec.date);
                        if (existingIdx !== -1) {
                            this.history[existingIdx] = rec;
                            updatedCount++;
                        } else {
                    ...
                    this.history.sort((a, b) => a.date.localeCompare(b.date));
                    showToast(`Activity history merged!\n• Updated: ${updatedCount} existing date(s)\n• Added: ${addedCount} new date(s)\n• Total: ${this.history.length} date(s) recorded.`, "success");
                } else {
                    this.history = cleanRecords.sort((a, b) => a.date.localeCompare(b.date));
                    showToast(`Activity history replaced!\n• Loaded ${this.history.length} daily activity record(s).`, "success");
                }

                this.saveToStorage();
                this.renderUI();
            } catch (err) {
                console.error("Failed to parse JSON Activity Trend file", err);
