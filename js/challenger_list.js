...
class ChallengerGenerator {
    ...
    async processSourceFile(file) {
        ...
        if (rawRows.length < 2) {
            showToast(`File "${fileName}" has no data rows!`, "error");
            return;
        }
        ...
        if (newRecords.length === 0) {
            showToast(`No Challenger DOs found in "${fileName}" (${skippedRecords.length} non-Challenger consignee row(s) skipped).`, "warning");
            return;
        }
        ...
        showToast(`Extracted ${newRecords.length} Challenger line(s) from "${fileName}"! Total lines: ${this.records.length}.`, "success");
    }
    ...
    async exportToExcel() {
        if (this.records.length === 0) {
            showToast("No Challenger data to export! Please upload source files first.", "warning");
            return;
        }
